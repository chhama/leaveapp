...
<thead>
  <tr>
    ...
    <th width="100" height="38" align="left">Type</th>
    <th width="121" class="text-center">Control</th>
  </tr>
  </thead>
  <tbody>
  	<?php $slno = 0; ?>
    ...
    <td align="left" class="action text-center">
        {{Form::open(array('url'=>route('user.destroy', array($user->id)),'method'=>'delete'))}}
            <a href="{{route('user.edit', array($user->id))}}" class="btn btn-fab btn-fab-mini btn-raised btn-sm btn-primary"  style="width:30px; height:30px; padding:11px 5px 10px 6px;" title="Edit User"><i class="glyphicon glyphicon-edit"></i></a>&nbsp;&nbsp;
            <button type="submit" onclick="return confirm ('<?php echo _('Are you sure') ?>');" name="id" class="btn btn-fab btn-fab-mini btn-raised btn-sm btn-danger" style="width:30px; height:30px; padding:11px 4px 10px 4px;" title="Remove User" value="{{$user->id}}"><i class="glyphicon glyphicon-trash"></i></button>
        {{Form::close()}}
    </td>
    </tr>
    <?php $slno++; ?>
